              ...
              <div className="flex-1 flex gap-4 h-full">
                ...
                <div className="w-64 flex-shrink-0 flex flex-col gap-3">
                  ...
                  {uploadedImage && (
                    <div className="theme-card rounded-xl p-3 flex flex-col">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs text-muted-foreground">模特原图</span>
                      </div>
                  ...
                  {sceneImagePreview && (
                    <div className="theme-card rounded-xl p-3 flex flex-col">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xs font-semibold text-blue-400 px-2 py-0.5 bg-blue-400/10 rounded-full">
                          素材B
                        </span>
                        <span className="text-xs text-muted-foreground">服装素材</span>
                      </div>
                      <div className="aspect-[3/4] bg-gray-900/50 rounded-lg overflow-hidden relative">
                        <img
                ...
                <div className="flex-1 theme-card rounded-2xl p-4 flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex-1 relative z-10 flex items-center justify-center">
                    <div className="relative w-full h-full flex items-center justify-center">
                      {/* 显示的图片 - 根据对比状态切换 */}
                      <img
                        src={`/api/image-proxy?url=${encodeURIComponent(isComparing && uploadedImage ? uploadedImage : generatedImage)}`}
                        alt={isComparing && uploadedImage ? "原图对比" : "AI生成结果"}
                        className="max-h-full max-w-full object-contain shadow-2xl rounded-lg"
                        onError={(e) => {
                          console.error('图片加载失败:', { src: (e.target as HTMLImageElement).src });
                        }}
                      />
                      ...
                      {uploadedImage && (
                        <button
                          ...
                          className="absolute top-4 right-4 bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 rounded-lg px-4 py-2 shadow-lg flex items-center gap-2 text-sm text-white transition-all hover:scale-105 active:scale-95"
                        >
                          <Eye size={16} />
                          <span>{isComparing ? '松开恢复' : '按住对比'}</span>
                        </button>
                      )}

                      {/* 对比状态指示器 - 左上角 */}
                      {isComparing && uploadedImage && (
                        <div className="absolute top-4 left-4 bg-blue-500/90 backdrop-blur-sm rounded-lg px-3 py-1.5 shadow-lg">
                      ...
                      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="btn-primary px-5 py-2 rounded-full shadow-lg flex items-center gap-1.5 text-sm transition-all hover:scale-105 active:scale-95"
                          title="下载AI生成图"
                        >
                        ...
                        {uploadedImage && (
                          <button
                            onClick={() => {
                              const proxyUrl = `/api/image-proxy?url=${encodeURIComponent(uploadedImage)}`;
                              fetch(proxyUrl)
                                .then(res => res.blob())
                                .then(blob => {
                                  const link = document.createElement('a');
                            ...
                            className="bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 px-5 py-2 rounded-full shadow-lg flex items-center gap-1.5 text-sm text-white transition-all hover:scale-105 active:scale-95"
                            title="下载原始图"
                          >
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">

# 双向状态同步预览设计

## 前端 ↔ 飞书表格 实时状态同步

### 场景1：前端用户操作时，飞书表格显示状态

```
┌─────────────────────────────────────────────────────────────────────┐
│                        飞书多维表格界面                              │
├─────────────────────────────────────────────────────────────────────┤
│  记录ID  │ 提示词       │ 状态    │ 最后修改者  │ 同步状态            │
├─────────────────────────────────────────────────────────────────────┤
│  rec001  │ 一件漂亮的裙子 │ 处理中 │ 🌐 Web用户  │ 🔄 同步中...        │
│          │               │         │ 2秒前编辑   │                    │
├─────────────────────────────────────────────────────────────────────┤
│  rec002  │ 商务西装      │ 已完成 │ 📊 飞书用户  │ ✅ 已同步          │
│          │               │         │ 5分钟前     │                    │
├─────────────────────────────────────────────────────────────────────┤
│  rec003  │ 休闲T恤      │ 待处理 │ 🤖 API自动   │ ⚠️ 冲突            │
│          │               │         │             │ 需要手动解决        │
└─────────────────────────────────────────────────────────────────────┘

"最后修改者"字段的图标说明：
🌐 = Web前端用户
📊 = 飞书表格用户
🤖 = 系统自动（API/N8N）
```

### 场景2：飞书用户操作时，前端显示状态

```
┌─────────────────────────────────────────────────────────────────────┐
│                           Web前端界面                                │
├─────────────────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │  任务卡片                                                      │  │
│  │  ┌────────────────────────────────────────────────────────┐  │  │
│  │  │ 一件漂亮的裙子                                          │  │  │
│  │  │                                                         │  │  │
│  │  │ ┌───────────────────────────────────────────────────┐ │  │  │
│  │  │ │ 🔄 实时协作提示                                     │ │  │  │
│  │  │ │                                                   │ │  │  │
│  │  │ │ 👤 飞书用户"张三" 正在编辑此任务                   │ │  │  │
│  │  │ │ 🕐 3秒前修改了"提示词"字段                         │ │  │  │
│  │  │ │                                                   │ │  │  │
│  │  │ │ [查看详情] [刷新]                                 │ │  │  │
│  │  │ └───────────────────────────────────────────────────┘ │  │  │
│  │  │                                                         │  │  │
│  │  │ 状态：处理中  │  进度：45%                             │  │  │
│  │  │ 最后修改：飞书用户  │  3秒前                           │  │  │
│  │  └────────────────────────────────────────────────────────┘  │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │  右下角浮动提示                                               │  │
│  │  ┌────────────────────────────────────────────────────┐      │  │
│  │  │ 👥 2 个任务正在被其他用户编辑                        │      │  │
│  │  │    点击查看详情                                      │      │  │
│  │  └────────────────────────────────────────────────────┘      │  │
│  └──────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 实现方案

### 1. 前端实时协作指示器组件

```typescript
// src/components/RealTimeCollaborationIndicator.tsx

interface CollaborationStatus {
  taskId: string;
  modifier: string;
  modifierType: 'web' | 'feishu' | 'api';
  modifiedAt: Date;
  modifiedField: string;
}

export function RealTimeCollaborationIndicator({ taskId }: { taskId: string }) {
  const [status, setStatus] = useState<CollaborationStatus | null>(null);
  const [showDetails, setShowDetails] = useState(false);

  useEffect(() => {
    // 订阅任务更新事件
    const eventBus = getEventBus();

    const unsubscribe = eventBus.on('task.external_update', (data) => {
      if (data.taskId === taskId && data.modifier !== 'web') {
        setStatus({
          taskId: data.taskId,
          modifier: data.modifierName,
          modifierType: data.modifierType,
          modifiedAt: new Date(data.modifiedAt),
          modifiedField: data.modifiedField,
        });

        // 5秒后自动隐藏
        setTimeout(() => setStatus(null), 5000);
      }
    });

    return unsubscribe;
  }, [taskId]);

  if (!status) return null;

  return (
    <Alert className="relative overflow-hidden">
      <div className="absolute inset-0 bg-blue-500/10 animate-pulse" />

      <div className="relative flex items-center gap-3">
        <Avatar className="h-8 w-8">
          <AvatarFallback className={cn(
            "bg-blue-500 text-white",
            status.modifierType === 'feishu' && "bg-green-500",
            status.modifierType === 'api' && "bg-gray-500"
          )}>
            {status.modifierType === 'web' && <Globe className="h-4 w-4" />}
            {status.modifierType === 'feishu' && <Grid3x3 className="h-4 w-4" />}
            {status.modifierType === 'api' && <Bot className="h-4 w-4" />}
          </AvatarFallback>
        </Avatar>

        <div className="flex-1">
          <p className="text-sm font-medium">
            {status.modifierType === 'feishu' && '飞书用户'}
            {status.modifierType === 'web' && 'Web用户'}
            {status.modifierType === 'api' && '系统'}
            "{status.modifier}" 正在编辑此任务
          </p>
          <p className="text-xs text-muted-foreground">
            {formatDistanceToNow(status.modifiedAt, { addSuffix: true })}
            修改了 <span className="font-medium">{status.modifiedField}</span>
          </p>
        </div>

        <div className="flex gap-2">
          <Button
            size="sm"
            variant="outline"
            onClick={() => setShowDetails(true)}
          >
            查看详情
          </Button>
          <Button
            size="sm"
            onClick={() => window.location.reload()}
          >
            刷新
          </Button>
        </div>
      </div>
    </Alert>
  );
}
```

### 2. 多用户编辑活动指示器

```typescript
// src/components/ActiveUsersPanel.tsx

interface ActiveUser {
  id: string;
  name: string;
  type: 'web' | 'feishu';
  avatar?: string;
  currentTask: string;
  activity: string;
}

export function ActiveUsersPanel() {
  const [activeUsers, setActiveUsers] = useState<ActiveUser[]>([]);

  useEffect(() => {
    // 订阅活跃用户列表
    const syncService = getSyncService();

    const unsubscribe = syncService.subscribeToActiveUsers((users) => {
      // 过滤掉当前用户
      setActiveUsers(users.filter(u => u.type !== 'web'));
    });

    return unsubscribe;
  }, []);

  if (activeUsers.length === 0) return null;

  return (
    <div className="fixed bottom-4 right-4 z-50">
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="secondary" className="shadow-lg">
            <Users className="h-4 w-4 mr-2" />
            {activeUsers.length} 个其他用户正在编辑
          </Button>
        </PopoverTrigger>

        <PopoverContent className="w-80" align="end">
          <div className="space-y-3">
            <h4 className="font-medium">正在编辑的用户</h4>

            <div className="space-y-2">
              {activeUsers.map(user => (
                <div
                  key={user.id}
                  className="flex items-center gap-3 p-2 rounded-lg bg-muted/50"
                >
                  <Avatar className="h-8 w-8">
                    <AvatarImage src={user.avatar} />
                    <AvatarFallback className={cn(
                      "bg-blue-500 text-white",
                      user.type === 'feishu' && "bg-green-500"
                    )}>
                      {user.name[0]}
                    </AvatarFallback>
                  </Avatar>

                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{user.name}</p>
                    <p className="text-xs text-muted-foreground truncate">
                      {user.activity}
                    </p>
                  </div>

                  <Badge variant="outline" className="text-xs">
                    {user.type === 'feishu' && '📊 飞书'}
                    {user.type === 'web' && '🌐 Web'}
                  </Badge>
                </div>
              ))}
            </div>
          </div>
        </PopoverContent>
      </Popover>
    </div>
  );
}
```

### 3. 任务卡片上的协作状态

```typescript
// src/components/TaskCard.tsx - 增强版

export function TaskCard({ task }: { task: Task }) {
  const [collaborationStatus, setCollaborationStatus] = useState<{
    isBeingEdited: boolean;
    modifier?: string;
  }>({ isBeingEdited: false });

  return (
    <Card className={cn(
      "transition-all",
      collaborationStatus.isBeingEdited && "ring-2 ring-blue-500"
    )}>
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <CardTitle className="text-base">
              {task.prompt?.slice(0, 50)}...
            </CardTitle>
          </div>

          {/* 协作状态徽章 */}
          <div className="flex items-center gap-2">
            {task.lastModifiedBy !== 'web' && (
              <Badge variant="secondary" className="text-xs">
                {task.lastModifiedBy === 'feishu' && '📊 飞书用户编辑'}
                {task.lastModifiedBy === 'api' && '🤖 系统更新'}
              </Badge>
            )}

            <SyncStatusIndicator
              taskId={task.id}
              lastModifiedAt={task.lastModifiedAt}
            />
          </div>
        </div>

        {/* 最后修改信息 */}
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <User className="h-3 w-3" />
          <span>
            {task.lastModifiedBy === 'web' && '你'}
            {task.lastModifiedBy === 'feishu' && '飞书用户'}
            {task.lastModifiedBy === 'api' && '系统'}
          </span>
          <span>·</span>
          <Clock className="h-3 w-3" />
          <span>{formatDistanceToNow(task.lastModifiedAt, { addSuffix: true })}</span>
        </div>
      </CardHeader>

      <CardContent>
        {/* 实时协作提示 */}
        {collaborationStatus.isBeingEdited && (
          <RealTimeCollaborationIndicator taskId={task.id} />
        )}

        {/* 任务内容 */}
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span>状态：</span>
            <Badge>{task.status}</Badge>
          </div>
          {task.progress > 0 && task.progress < 100 && (
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span>进度</span>
                <span>{task.progress}%</span>
              </div>
              <Progress value={task.progress} />
            </div>
          )}
        </div>
      </CardContent>
    </Card>
  );
}
```

### 4. 飞书表格状态同步

#### 飞书表格字段配置

```javascript
// 飞书多维表格字段定义

const feishuTableFields = {
  // 基础字段
  记录ID: 'record_id',
  提示词: 'text',
  状态: 'select',
  创建时间: 'created_time',

  // 新增：协作状态字段
  最后修改者: {
    type: 'text',
    readonly: true,
    formula: `
      SWITCH(
        lastModifiedBy,
        "web", "🌐 Web用户",
        "feishu", "📊 飞书用户",
        "api", "🤖 系统自动",
        "未知"
      )
    `,
  },

  最后修改时间: {
    type: 'datetime',
    readonly: true,
    field: 'lastModifiedAt',
  },

  同步状态: {
    type: 'select',
    readonly: true,
    options: ['✅ 已同步', '🔄 同步中', '⚠️ 冲突', '❌ 同步失败'],
  },

  当前编辑者: {
    type: 'text',
    readonly: true,
    description: '正在编辑此记录的用户',
  },
};
```

#### 飞书表格条件格式（高亮正在编辑的记录）

```javascript
// 飞书表格自动化规则

const activeEditingHighlight = {
  name: '高亮正在编辑的记录',
  condition: {
    field: '同步状态',
    operator: 'is',
    value: ['🔄 同步中'],
  },
  format: {
    background_color: '#DBEAFE', // 蓝色背景
    border_color: '#3B82F6',
    border_width: 2,
  },
};

const conflictHighlight = {
  name: '高亮冲突记录',
  condition: {
    field: '同步状态',
    operator: 'is',
    value: ['⚠️ 冲突'],
  },
  format: {
    background_color: '#FEE2E2', // 红色背景
    border_color: '#EF4444',
    border_width: 2,
  },
};
```

### 5. WebSocket实时同步服务

```typescript
// src/lib/services/realtime-sync.service.ts

export class RealTimeSyncService {
  private ws: WebSocket | null = null;
  private subscribers: Map<string, Set<Function>> = new Map();

  /**
   * 连接WebSocket
   */
  connect() {
    const wsUrl = process.env.NEXT_PUBLIC_WS_URL;
    this.ws = new WebSocket(wsUrl);

    this.ws.onmessage = event => {
      const data = JSON.parse(event.data);
      this.handleMessage(data);
    };
  }

  /**
   * 订阅任务更新
   */
  subscribeToTask(taskId: string, callback: Function) {
    if (!this.subscribers.has(taskId)) {
      this.subscribers.set(taskId, new Set());
    }
    this.subscribers.get(taskId)!.add(callback);

    // 发送订阅消息
    this.send({
      type: 'subscribe',
      taskId,
    });

    // 返回取消订阅函数
    return () => {
      this.subscribers.get(taskId)?.delete(callback);
      this.send({
        type: 'unsubscribe',
        taskId,
      });
    };
  }

  /**
   * 订阅活跃用户列表
   */
  subscribeToActiveUsers(callback: (users: ActiveUser[]) => void) {
    this.ws.onmessage = event => {
      const data = JSON.parse(event.data);
      if (data.type === 'active_users') {
        callback(data.users);
      }
    };
  }

  /**
   * 广播任务更新
   */
  broadcastTaskUpdate(task: Task, modifiedBy: string) {
    this.send({
      type: 'task_update',
      taskId: task.id,
      task,
      modifiedBy,
      timestamp: new Date().toISOString(),
    });
  }

  /**
   * 处理接收到的消息
   */
  private handleMessage(data: any) {
    switch (data.type) {
      case 'task_update':
        // 通知订阅者
        const subscribers = this.subscribers.get(data.taskId);
        if (subscribers) {
          subscribers.forEach(callback => callback(data));
        }

        // 发送到事件总线
        const eventBus = getEventBus();
        eventBus.emit('task.external_update', {
          taskId: data.taskId,
          modifierName: data.modifiedBy,
          modifierType: data.task.lastModifiedBy,
          modifiedAt: data.timestamp,
          modifiedField: data.changedField,
        });
        break;
    }
  }

  /**
   * 发送消息
   */
  private send(data: any) {
    if (this.ws?.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify(data));
    }
  }
}
```

### 6. API端点：活跃用户查询

```typescript
// src/app/api/active-users/route.ts

export async function GET(req: NextRequest) {
  const { searchParams } = new URL(req.url);
  const taskId = searchParams.get('taskId');

  // 查询活跃用户
  const activeUsers = await getActiveUsers(taskId);

  return NextResponse.json({
    users: activeUsers.map(user => ({
      id: user.id,
      name: user.name,
      type: user.lastModifiedBy === 'web' ? 'web' : 'feishu',
      currentTask: user.currentTaskId,
      activity: `${user.action} "${user.modifiedField}"`,
    })),
  });
}
```

---

## 总结

### 前端显示飞书操作状态

- ✅ 任务卡片上的"最后修改者"徽章
- ✅ 实时协作提示（Alert组件）
- ✅ 右下角活跃用户指示器
- ✅ WebSocket实时更新

### 飞书显示前端操作状态

- ✅ "最后修改者"字段（带图标）
- ✅ "同步状态"字段（已同步/同步中/冲突）
- ✅ 条件格式高亮正在编辑的记录
- ✅ "当前编辑者"字段

### 用户体验

- Web用户可以看到飞书用户正在编辑的任务
- 飞书用户可以看到Web用户正在编辑的记录
- 双向实时同步，延迟 < 1秒
- 冲突时提供明确的视觉提示
